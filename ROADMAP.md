...
337. **`/session list --output-format json` session lifecycle objects expose `workspace_dirty: true` but provide no detail about which files are dirty or how many, making the flag unactionable for GC/cleanup decisions** — dogfooded 2026-04-29 by Jobdori on current main (`0f7578c`). Running `claw --output-format json --resume latest /session list` returns `session_details` where every `lifecycle` object for non-current sessions shows `"workspace_dirty": true, "pane_id": null, "pane_command": null, "pane_path": null` — and no `dirty_files`, `dirty_file_count`, `git_status`, or `uncommitted_changes` field. A session GC policy or cleanup lane that wants to prune old sessions cannot determine: (a) whether the dirty state is intentional (user edits in progress) or stale (leftover from a crash), (b) how many files are affected, (c) whether the dirty files are tracked by git or untracked, (d) what the workspace head commit was when the session was created. Additionally, `"abandoned": true` appears on most saved sessions with no definition of the abandonment criterion: was the session killed? did the pane close? did the user interrupt? Without this context, `abandoned` is a binary label with no actionable signal. **Required fix shape:** (a) add `dirty_file_count: u32` to lifecycle objects when `workspace_dirty = true`; (b) optionally add `dirty_file_paths: Vec<String>` (or summarized `["path/a.rs", "path/b.rs", "... +N"]`) so callers can assess severity; (c) add `abandoned_reason: "pane_closed" | "process_killed" | "session_replaced" | null` to explain the abandonment source; (d) add regression coverage proving that `workspace_dirty = true` sessions always include at least `dirty_file_count`. **Why this matters:** without dirty-file detail, an orchestration lane that wants to safely GC old sessions must either skip all dirty sessions (leaks disk) or wipe all dirty sessions (risks destroying work in progress). Source: Jobdori live dogfood on mengmotaHost, claw-code `0f7578c`, 2026-04-29.
